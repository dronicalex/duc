...
let burgerMenu = document.querySelector('.header__body')
...
if (isMobile.any()) {
	...
	window.addEventListener('scroll', function () {
		let q = window.scrollY / 7
		let a = window.scrollY / 4
		footerBg.forEach((item) => {

			item.style.transform = "translate(" + 0 + "%, -" + q + "%) rotate(" + a + "deg )";
			q += 2
			a += 50
		});
	})
	...
	// 	footerBgImg.forEach((item) => {
	// 		// console.log(item.offsetTop)


	// 		item.style.transform = "translate(" + 0 + "px, " + q / 5 + "px) ";
	// 		// q += 5
	// 		console.log(q)
	// 	})
	// })
...
function paralaxHuman() {
	...
	for (let index = 0; index < human.length; index++) {
		...
		element.style.transform = "translate(" + (positionX / forHuman) + "%, " + (positionY / forHuman) + "%) rotate(" + (positionX / forHuman) + "deg )";
		forHuman -= 2
		speed += 0.5;
...
let monets = document.querySelectorAll('._moneta')
let pero1 = document.querySelector('.pero1');
let pero2 = document.querySelector('.pero2');
window.addEventListener('scroll', function () {
	let y = window.scrollY

	for (let index = 0; index < monets.length; index++) {
		const element = monets[index];

		element.style.transform = "translate( 0px, " + (y / 11) + "px)"
	}
	pero1.style.transform = "translate( 0px, -" + (y / 8) + "px)"
	pero2.style.transform = "translate( 0px, -" + (y / 5) + "px)"
})




let storyImg = document.querySelectorAll('.story__img img')
let change = document.querySelector('.story__btn')

for (let index = 0; index < storyImg.length; index++) {
	const element = storyImg[index];
	element.classList.add('_dn')
}
storyImg[0].classList.remove('_dn')

change.addEventListener('click', function () {
	for (let index = 0; index < storyImg.length; index++) {
		const element = storyImg[index];
		if (!element.classList.contains('_dn')) {
			element.classList.add('_dn')

			if (storyImg[index] == storyImg[storyImg.length - 1]) {
				storyImg[0].classList.remove('_dn')
				return
			} else {
				storyImg[index + 1].classList.remove('_dn')
				return
			}
...
// -------------------------timer---------------------
let endDate = new Date("May 27, 2022 22:00:00").getTime();
let timer = setInterval(function () {
	...
	if (t >= 0) {
		...
		document.querySelector("._day").innerHTML = days
		if (hours.toString().length == 1) {
			document.querySelector("._hour").innerHTML = ("0" + hours)
		} else {
			document.querySelector("._hour").innerHTML = hours
		}
		if (mins.toString().length == 1) {
			document.querySelector("._minutes").innerHTML = ("0" + mins)
		} else {
			document.querySelector("._minutes").innerHTML = mins
		}
